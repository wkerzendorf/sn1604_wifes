...
\tablehead{\colhead{Name} & \colhead{RA} & \colhead{Dec} & \colhead{F550M} & \colhead{Luminosity(V)\tablenotemark{a}}
& \colhead{NOMAD ID}\tablenotemark{b} & \colhead{B\tablenotemark{d}} & \colhead{V\tablenotemark{d}} & \colhead{R\tablenotemark{d}}
 & \colhead{J\tablenotemark{d}} & \colhead{H\tablenotemark{d}} & \colhead{K\tablenotemark{d}}\\
...
\enddata
\tablenotetext{a}{Assuming the candidate to be at the distance of SN1604 by using the distance modulus calculated from \sn{1604}'s apparent magnitude and average \sneia\ absolute magnitude we obtain $m_V - M_V=16.3$. If we, instead, assumed a distance of 6~kpc and an extinction of $A_V=2.8$
the luminosity increases by a factor of 1.4 ($m_V - M_V = 16.7$)}
\tablenotetext{b}{In addition to stars marked with the same letter (but different digit), the extraction Region B and D are also covered by one NOMAD ID}
\tablenotetext{c}{Candidate for follow-up ($L>20\lsun$ and $P(\textrm{donor}|\vrad)>1\%$)}
\tablenotetext{d}{Photometry from the NOMAD catalogue (accuracy $\approx 0.3~\textrm{mag}$)}
\end{deluxetable}
